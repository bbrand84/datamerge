...
public class CRFAppl {
	...
	public static void main(String argv[]) throws Exception {
		/*
		 * Initialization: Get the required arguements for the application here.
		 * Also, you will need to create a Properties object for arguements to
		 * be passed to the CRF. You do not need to worry about this object,
		 * because there are default values for all the parameters in the CRF
		 * package. You may need to pass your own parameters values for tuning
		 * the application performance.
		 */

		/*
		 * There are mainly two phases for a learning application: Training and
		 * Testing. Implement two routines for each of the phases and call them
		 * appropriately here.
		 */
		CRFAppl crfa = new CRFAppl();
		crfa.options = new Properties();		
		crfa.options.put("modelGraph", "naive");// http://crf.sourceforge.net/introduction/models.html
		crfa.options.put("numLabels", 7);
		crfa.options.put("delimit", " \t"); // used to define token boundaries
		crfa.options.put("tagDelimit","|"); // separator between tokens and tag number
